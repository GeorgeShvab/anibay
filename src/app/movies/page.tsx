...
const Movies: FC<PageProps<{ page?: string; order_by?: string }>> = async ({ searchParams }) => {
  ...
  const moviesPromise = AnimeService.getMovies({
    page: page < 1 ? 0 : page - 1,
    orderBy: searchParams.order_by === 'rating' ? 'rating' : 'popularity',
  })
  const hasNextPagePromise = AnimeService.getMovies({
    page: page < 1 ? 1 : page,
    orderBy: searchParams.order_by === 'rating' ? 'rating' : 'popularity',
  })
...
export const metadata: Metadata = {
  title: `Movies`,
  description: `Thousands of subbed anime movies in a good quality. Completely free.`,
  openGraph: {
    images: ['/auth-bg.png'],
    title: 'Movies',
    description: 'Thousands of subbed anime movies in a good quality. Completely free.',
